...
from database_functions.login_functions import is_admin, already_auth, check_worker_code, put_data, check_admin_code
...
async def start(message: Message):
    if already_auth(message.chat.id, path_to_database_users):
        if is_admin(message.chat.id, path_to_database_users):
            await message.answer('Вы успешно авторизовались, как админ', reply_markup=admin_keyboards.main())
        else:
            await message.answer('Вы успешно авторизовались, как сотрудник', reply_markup=user_keyboards.main())
    else:
